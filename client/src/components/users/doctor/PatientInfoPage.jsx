  ...
  useEffect(() => {
    const fetchPatientData = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3000/auth/doctor/patient/${patientID}`, {
          headers: {
          ...
          <ul className="list-disc list-inside">
            {disabilities.map((disability) => (
              <li key={disability.disabilityID}>
                <strong>{disability.disabilityType}</strong> - {disability.notes}
              </li>
            ))}
          </ul>
        </div>
      ...
      {prescriptions && prescriptions.length > 0 && (
        <div className="mt-6">
          ...
          {prescriptions.map((prescription) => (
            <div key={prescription.prescriptionID} className="border p-4 mb-4">
              <p><strong>Medication:</strong> {prescription.medicationName}</p>
              <p><strong>Dosage:</strong> {prescription.dosage} {prescription.frequency}</p>
              <p><strong>Instructions:</strong> {prescription.instruction}</p>
              <p><strong>Status:</strong> {prescription.activeStatus}</p>
            </div>
          ))}
        </div>
      )}
        ...
        <div className="mt-6">
          ...
          {vitals.map((vital) => (
            <div key={vital.vitalsId} className="border p-4 mb-4">
              ...
              <p><strong>Blood Pressure:</strong> {vital.bloodPressureSystolic}/{vital.bloodPressureDiastolic} mmHg</p>
              <p><strong>Heart Rate:</strong> {vital.heartRate} bpm</p>
              <p><strong>Body Temperature:</strong> {vital.bodyTemperature} °C</p>
              <p><strong>Respiratory Rate:</strong> {vital.respiratoryRate} breaths per minute</p>
      ...
      {familyHistory && familyHistory.length > 0 && (
        <div className="mt-6">
          ...
          {familyHistory.map((history) => (
            <div key={history.familyHistoryID} className="border p-4 mb-4">
