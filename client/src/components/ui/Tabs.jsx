...
export const Tabs = ({ children, defaultValue, ...props }) => {
  ...
  return (
    <TabsContext.Provider {...props} value={{ activeTab, setActiveTab }}>
      <div>{children}</div>
...
export const TabsTrigger = ({ value, children, ...props }) => {
  ...
  return (
    <button
      {...props}
      className={` flex items-center py-1 px-20 ${
        isActive
...
export const TabsList = ({ children, ...props }) => {
  return (
    <div
      {...props}
      className="grid w-full grid-cols-4 bg-pink-100 p-1 rounded-lg"
    >
      {children}
    </div>
  );
