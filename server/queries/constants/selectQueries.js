...
export const GET_PREVIOUS_APPOINTMENTS = `SELECT a.appointmentID, a.afterAppointmentNotes, a.appointmentDateTime AS date, s.serviceName, CONCAT(d.firstName, ' ', d.lastName) AS doctorFullName, sp.specialtyName, o.officeName, CONCAT(addr.addrStreet, ' ', addr.addrcity, ' ', addr.addrstate, ' ', addr.addrzip) AS officeAddress FROM appointment a INNER JOIN service s ON a.serviceID = s.serviceID INNER JOIN doctor d ON a.doctorID = d.doctorID INNER JOIN specialty sp ON d.specialtyID = sp.specialtyID INNER JOIN office o ON a.officeID = o.officeID INNER JOIN address addr ON o.addressID = addr.addressID INNER JOIN patient p ON a.patientID = p.patientID WHERE p.patientID = ? AND a.status = 'Completed';`;
